// Admin dashboard entry point: sign-in, testimonial review, edit/approve/
// delete, search/sort/filter, and CSV export.
//
// Architecture in one paragraph: on load we ask /api/admin/session whether a
// valid cookie exists. If yes → dashboard shows, if no → login form shows.
// Every request goes through the shared request() helper. Writes send both an
// X-CSRF-Token header (the server compares it to the session's token) and the
// admin Origin — both are required by the API, and both come free from being
// same-origin. All HTML strings built here escape user content via escapeHtml.

import '../../shared/style.css';
import './style.css';
import { el, icon, escapeHtml, avatar, dateLabel, dateTimeLabel, request, ApiError, type Page, type Testimonial } from '../../shared/ui';

// Static decorative icons (lock, arrow, ...) are injected once at startup.
...
// The header "View website" link; VITE_WEBSITE_URL is public build-time config.
el<HTMLAnchorElement>('#website-link').href = import.meta.env.VITE_WEBSITE_URL || 'http://localhost:5173';

// --- Module-level state ---------------------------------------------------
let csrfToken = '';                                     // Echoed back on writes.
let currentPage = 1;
let status = 'all';                                     // Active status filter tab.
let items: Testimonial[] = [];                          // Opaque source for the table.
let selected: Testimonial | null = null;                // The row an open dialog edits/deletes.
let requestId = 0;                                      // Guards against stale async results.
let toastTimer: ReturnType<typeof setTimeout>;          // Auto-hide timer for toasts.
const editDialog = el<HTMLDialogElement>('#edit-dialog');
const deleteDialog = el<HTMLDialogElement>('#delete-dialog');

// Small transient message in the bottom corner; `error=true` restyles it red.
...
// Reset the UI to the login screen. Used on logout AND when any request
// returns 401 (expired session), so the user is always routed back to sign-in.
function showLogin(message = '') {
  clearTimeout(searchTimer);
  requestId++; // Invalidate any in-flight list request (its id is now stale).
  csrfToken = ''; items = []; selected = null;
...
// Render the dashboard for a valid session and load the first testimonial page.
...
// Turns errors into a safe message and, for 401s, bounces to login.
...
// Serialise the current controls into the query string the API list expects.
function query() {
...
// One <tr> for the table: avatar+name+role, quote preview, status badge,
// received date, and action buttons (Review/Approve/Delete). Every dynamic
// value is escaped; status maps to a human label.
...
// Fetch and render the current page of the table (text response + stats).
async function loadList() {
  if (!csrfToken) return; // No session yet — nothing to load.
  const id = ++requestId; // Newest call wins; older ones are ignored on return.
  ...
  try {
    ...
    // If the last item in the current page was deleted/archived and we're not
    // on page 1, step back one page rather than show an empty page.
    ...
    // Render the table body (or the appropriate empty state with an anchor to
    // either adjust the search or note that everything is reviewed).
    ...
    // Pagination summary + disabling prev/next at the edges.
    ...
    if (!csrfToken) return; // handleError() logged us out; don't pile on.
...
// Writes (login/logout/edit/approve/delete) all carry Content-Type: application/
// json and the CSRF token. The Origin header is added by the browser from the
// page URL — same-origin /api, so it always matches the server's allowlist.
...
// --- List controls: filter tabs, search, sort, pagination ------------------
document.querySelectorAll<HTMLButtonElement>('[data-status]').forEach(button => button.addEventListener('click', () => {
  ...
  // aria-pressed marks the active tab for assistive tech.
...
// --- Row actions (event delegation on the whole table) --------------------
// A single listener on the container catches clicks on any action button —
// we never re-attach listeners when the table re-renders.
el('#table-container').addEventListener('click', async event => {
  ...
  // Quick "approve" without opening the dialog (one click, straight to live).
  if (button.dataset.action === 'approve') {
    button.disabled = true;
    try {
      // Send the whole item back (PATCH overwrites all fields) with status=approved.
  ...
  selected = item; // Remember which testimonial a dialog below operates on.
  ...
  // Edit: pre-fill every editable field with the current values.
  ...
  // Show the private submission details the visitor sent in (location,
  // timestamp, timezone). Needs escaping because they are untrusted data.
...
el<HTMLFormElement>('#edit-form').addEventListener('submit', async event => {
  ...
  // new FormData(form) collects the named fields (name, view, company, ...)
  // exactly as the API's edit schema expects.
...
el('#export').addEventListener('click', async () => {
  ...
  try {
    // Export is a file download, not JSON, so it bypasses request() and uses
    // fetch directly; the CSV arrives as a Blob we save via a temporary <a>.
...
// Decide login-vs-dashboard once, up front. 401 (no/invalid cookie) just shows
// the login form; anything else is surfaced as text under the form.
